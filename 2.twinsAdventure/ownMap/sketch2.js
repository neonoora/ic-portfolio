...
function preload(){
  sisb = loadImage("images/l1f2.png")
...
// LEVEL FAIL BEGINS
      ...
      if(window.parent.gameState == 1.2 && window.parent.replay == true){
              for(let i = 0; i<heartsL1.length; i++){
                heartsL1[i].x = int(random(1,10))
                heartsL1[i].y = int(random(1,10))
              }
              for(let i = 0; i<gnomesL1.length; i++){
        ...
        if(window.parent.gameMode == 1000){
              for(let y = 0; y < 11; y ++){
                  for(let x = 0; x < 11; x ++){
                    ...
                    art = road
                    image(art,x*50,y*50,50,50)
                    if(window.parent.world_cus2[y][x]== 1){
                      art = tree
                      image(art,x*50,y*50,50,50)
                    }
                    if(window.parent.world_cus2[y][x]== 22){
                        art = door
                        image(art,x*50,y*50,50,50)
                    }
                    if(window.parent.world_cus2[y][x]== 222){
                        art = sisb
                        image(art,x*50,y*50,50,50)
                      }
                  }
...
// SHOW CUSTOMIZE LEVEL BEGINS
        if(window.parent.gameMode == 2000){
          // image(sisb,window.parent.c2PosX,window.parent.c2PosY,50,50)
          cPosX = window.parent.c2PosX
          cPosY = window.parent.c2PosY
        }
...
class SIS2 {
    ...
    getHeartL1(){
      if(getOK(getTileByPos(this.x,this.y)) == true){
        ...
        for (let i = 0; i<heartsL1.length; i++){
          if(heartsL1[i].x == getTileByPos_X(this.x,this.y)
          && heartsL1[i].y == getTileByPos_Y(this.x,this.y)){
            heartsL1.splice (heartsL1[i],1)
            lifeL1 +=1
          }
        }
      }
    ...
    hitGnomeL1(){
      if(minusOK(getTileByPos(this.x,this.y)) == true){
        window.parent.world2[getTileByPos_Y(this.x,this.y)][getTileByPos_X(this.x,this.y)]= 9
        for (let i = 0; i<gnomesL1.length; i++){
          if(gnomesL1[i].x == getTileByPos_X(this.x,this.y)
          && gnomesL1[i].y == getTileByPos_Y(this.x,this.y)){
            gnomesL1.splice (gnomesL1[i],1)
              if(lifeL1 > 1){
                lifeL1 -= 1
              }else{
                lifeL1 = 0
    ...
    hitGnomeL2(){
      if(minusOK(getTileByPos(this.x,this.y)) == true){
        window.parent.world2[getTileByPos_Y(this.x,this.y)][getTileByPos_X(this.x,this.y)]= 9
        for (let i = 0; i<gnomesL2.length; i++){
          if(gnomesL2[i].x == getTileByPos_X(this.x,this.y)
          && gnomesL2[i].y == getTileByPos_Y(this.x,this.y)){
            gnomesL2.splice (gnomesL2[i],1)
              if(lifeL2 > 1){
                lifeL2 -= 1
              }else{
                lifeL2 = 0
    ...
    // exit if enter the door
          ...
          exitL1 (){
            if(window.parent.gameState == 1 &&  exitOK(getTileByPos(this.x,this.y)) == true && lifeL1 > 0){
              window.parent.gameState = 2
          ...
          exitL2(){
            if(window.parent.gameState == 2 &&  exitOK(getTileByPos(this.x,this.y)) == true && lifeL2 > 0){
              window.parent.gameState = 3
          ...
          exitL3(){
            if(window.parent.gameState == 3 &&  exitOK(getTileByPos(this.x,this.y)) == true && lifeL3 > 0){
